function [U_f,U_r,min_Pwr] = EVNMPC_laodtransfer(req_data, current_timestep,SOC, v_curr,Torque_demand, EMspeed)
    ...
    %function [F_aero, F_rr]=EVstates(SOC,u,N,Ts,veh,v_k,v_ref, info)
    %    F_aero = (veh.rho*veh.A*veh.Cd*(v_k^2))/2;
    %    F_rr = veh.M*9.81*veh.Crr;
    %    F_trac = F_aero + F_rr + veh.M*(v_ref(i) - v_k)/Ts;
    %    torque_demand = F_trac*veh.R_whl;
